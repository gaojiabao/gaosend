...
void MD5DigestDisplay(unsigned char* array) 
{
	int i;
	for(i=0; i<16; i++) {
		printf("%02X", array[i]);
...
void UseTimesFunction(int iNum)
{
	int		iUseFd;
	char	cUseNumber[10];
...
void CertificationAuthority()
{
	...
	if(iUseNumber > MAXUSETIMES){
		LOGRECORD(WARNING, "The frequency of use is over limited");
	}
